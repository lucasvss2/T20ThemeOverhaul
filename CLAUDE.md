...
### Poções e Pergaminhos + identificação (v1.104.0)
...
- **Uso** (`src/pocoes-pergaminhos/index.ts`): NÃO reimplementa a magia — clona a magia REAL via `fromUuid(spellUuid)` (mesmo padrão de `castGolpeSpell` em `golpe-pessoal/index.ts`), zera `system.ativacao.custo` (usar não custa PM — já pago na fabricação; **sem o floor nativo `max(custo,1)`** porque `hasManaCost` do T20 lê o custo ORIGINAL do clone, que aqui já nasce 0, ao contrário do caso do Golpe Pessoal que precisa de refund pós-hoc), importa temporariamente na ficha (flag `pocaoPergaminhoTemp`) e dispara `.roll()` nativo nela — reaproveita 100% da automação (spell-resistance, conditions-map, area-engine) sem código de dano/resistência aqui. Item temporário apagado ~2 min depois (mesma janela do Golpe Pessoal); consome 1 uso do item físico (`qtd -=1`, deleta ao chegar em 0) logo após disparar o roll.
  ...
  - **Patch em `AbilityUseDialog.create`** (encadeável, mesmo padrão de Baforada/Cruzado/Bolsa de Pó/Golpe Pessoal): item com a flag `pocaoPergaminho` sempre retorna `null` (cancela o fluxo nativo — nosso item não tem `rolls` próprios, não haveria nada útil a rolar) e despacha pro identificar-ou-conjurar.
...
- **Verificado ao vivo (arton):** harvest real rodado (264 magias, 261 únicas), pack compilado e servido (`main.bundle.js` confirmado via fetch direto contendo "Pata Inseto"/gold-split — bundle novo realmente ativo apesar do `game.modules.get(...).version` ainda mostrar a versão antiga em cache do mundo, cosmético, não afeta o JS servido). Teste manual de masking com item fabricado à mão na ficha do Al Simmons não pegou o hook novo por rodar ANTES do reload da aba pegar o bundle atualizado — pack em si (`pocoes-pergaminhos`) só é reconhecido pelo Foundry após um RELAUNCH do mundo (mudança na entrada `"packs"` do `module.json`, mesmo gotcha documentado pro `packs/ameacas`), que exige a senha de administrador do servidor (GM2/Trusted não tem esse privilégio — `POST /setup` retornou 403) — **verificação end-to-end completa de B3/B4/B5 fica pendente até o usuário relançar o mundo** (Configuração → Retomar o Mundo).
