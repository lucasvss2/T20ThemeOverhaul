...
### Orientação (v1.100.0, nome corrigido v1.100.1)
...
- **⚠️ "Não funciona" que na real é design correto:** usuário reportou que o buff aplicava mas não gerava `2d20` — a rolagem de teste era **Fortitude**, excluída de propósito (teste de resistência, não de perícia — ver acima). Confirmado pelo log de chat do próprio mundo (`formula:"1d20 + 3 + 0"` na Fortitude, logo após o card do buff). Não é bug — é a regra "não se aplica a... resistência" funcionando.
- **Verificado ao vivo (Al Simmons, Arcanista 3), duas rodadas de teste (console + clique real na ficha/HUD, antes e depois do v1.101.0):** base sem aprimoramento → card postado, AE `mode:"once"` no alvo, próximo teste de perícia elegível (Furtividade/Percepção) saiu `2d20kh` (fórmula e resultado dos dois dados capturados no chat), buff consumido depois do uso; +2 PM (escolha "Força") → AE `mode:"persistent", scopeAttrs:["for"]`, Atletismo (For) saiu `2d20kh` e a AE PERMANECEU (não consome), Furtividade (Des, fora do escopo) saiu `1d20` normal; Fortitude sempre `1d20` (exclusão correta). Pós-refatoração pro registro compartilhado: reconfirmado `2d20kh` em Percepção via clique real na ficha (14 descartado, 18 mantido, total 28).
