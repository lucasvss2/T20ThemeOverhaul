...
### Modo mobile da HUD — Fase 1: fundação (v1.105.0)
...
- **CSS desta fase** (`hud.css`): com `.t20-mobile-mode` no `<html>`, esconde o chrome desktop do Foundry (`#board` canvas, `#sidebar`, `#scene-controls`, `#players`, `#navigation`) e remove o cap de largura da HUD (`.t20-hud-root{width:100%}`, era `min(680px,100%)`).
- **⚠️ Escopo desta fase, de propósito:** a HUD ainda NÃO se reestrutura verticalmente pra ocupar a tela toda — ela continua ancorada como rodapé (`align-items:flex-end` em `#hotbar`), só mais larga e sem o canvas atrás. Empilhar retrato/orbes no topo + perícias OU painel direito como abas únicas de tela cheia (Perícias virando uma 5ª aba ao lado de Inventário/Poderes/Magias) é a **Fase 2 (M5)** — adiada de propósito por exigir restructurar um grid que hoje funciona bem (o layout lado-a-lado atual), e que eu não tenho como verificar visualmente nesta sessão (sem acesso de navegador à sessão Foundry real do usuário). Não shippar essa parte sem verificação ao vivo primeiro.
