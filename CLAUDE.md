...
- **Arte dos tokens — bloqueada, prompts entregues em texto:** ver seção "Gem do Gemini pra retrato de ameaça" abaixo — os 28 `img`/`texture.src` apontam pra caminhos em `public/assets/{Thartan,Deserto da Perdição,Avulsos,Ahlen}/<Nome>.png` que **ainda não existem em disco** (ícone quebrado no Foundry até o usuário colocar os PNGs manualmente — ele vai gerar via Gemini e avisar).
- **Deploy:** só mudou `packs-src/ameacas/` (dados) — nenhum código `src/` novo, `npm run typecheck && npm test && npm run build` continuam de controle (nada quebrou, só confirma que o resto do bundle está intacto).
