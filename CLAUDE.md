...
- **Finalizar** → importa os poderes escolhidos (busca por nome em TODOS os packs de Item, sistema primeiro — `loadPowerDocs`), cria complicação `"Obrigações e Restrições — <Deus>"` (poder, subtipo "Complicação", flag `divindadeComplicacao {deus, automacao}`), seta `system.detalhes.divindade`, posta card. **Cancelar/fechar sem finalizar → DELETA o item** (escolha abortada). 2ª divindade → warn + delete do novo item.
- **deleteItem da divindade** → remove complicação + AE Nimb + limpa `detalhes.divindade`. Poderes concedidos FICAM (decisão: mestre remove).
