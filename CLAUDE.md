...
src/
  ...
  hud/skill-icons.ts           — 29 ícones SVG de perícia portados do Claude Design (por label PT-BR)
...
- **Combate** (`combat-toggle.ts`): API confirmada lendo `client/documents/combat.mjs` — `combat.started = round>0`; `combat.endCombat()` já abre `DialogV2.confirm` nativo e deleta o documento; criar do zero replica `CombatTracker#_onCombatCreate` (`Combat.create()` + `activate({render:false})` + `startCombat()`). "Finalizar Turno" só aparece pro combatente cujo `combatant.players` (getter derivado, não array salvo) inclui `game.user`. **O indicador visual "Em Combate/Fora de Combate" foi removido da UI** por pedido do usuário — a lógica (`getCombatState`/`toggleCombatState`) permanece testada e disponível.
- **Retrato**: quadrado (não circular) com `background-size:cover`, nome sobreposto num gradiente escuro na base (`.t20-hud-portrait-name`), não mais um `<div>` de nome separado acima.
- **Stepper de linhas** (`state.ts`): `rows` (1-4) persiste via client setting `hud.rows` (`scope:"client", config:false`).
