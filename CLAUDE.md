...
- **Alvos:** o bardo (`getActiveTokens()[0]`) SEMPRE + `game.user.targets` cujo token esteja a **≤6 quadrados** (`squaresBetween`, **Chebyshev** em quadrados = `Δpx/grid.size`). ⚠️ Medir em QUADRADOS, não em `grid.distance` — a escala da cena varia (achado ao vivo: cena "Candeh'ssa" com `grid.distance:50`, não 1,5 m; converter p/ metros quebraria o alcance). Alvos T fora do alcance → `ui.notifications.warn`.
...
- **Adamante** (instrumento equipado): +1 no bônus final. Detecta por `system.upgrades.material==="adamant"` (checagem do [[adamante]]) **OU pelo NOME** (`norm(name).includes("adamant")`) — ⚠️ achado ao vivo: "Gaita de Foles de Adamante" trazia o material só no nome (`upgrades.material:""`). Gate por nome de instrumento (`INSTRUMENT_NAMES`).
...
- **Fases seguintes (NÃO implementadas):** poderes Arte Mágica (+2 CD de habilidades de bardo enquanto sob Inspiração) e Inspiração Espirituosa (PM temp = bônus, 1×/combate, nv 11); instrumentos Clarim/Cítara/Cornamusa/Flauta/Tamborete/Violino (Trombeta dispensada); materiais Madeira Tollon/Matéria Vermelha/Mitral.
