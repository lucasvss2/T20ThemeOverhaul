...
src/
  ...
  orientacao-divina/index.ts   — Orientação Divina: vantagem em teste de perícia (patch rollPericia → event.altKey); +tiers escopo por atributo/grupo (cena) + alvos múltiplos
  ...
  hud/orb.ts                   — Orbes PV/PM: clique=dano/gasto, Shift+clique=cura/recupera, floaters, update otimista
...
- **Mecanismo (vantagem em teste de perícia):** ⚠️ `Actor.prototype.rollPericia` **NÃO** propaga `options.rollKeep` pro `d20Roll` como `Item.prototype.rollAttack` faz (esse merga `options` inteiro em `rollConfig`; `rollPericia` só repassa `options.event`/`options.message`, o resto de `rollConfig` vem do dialog/`applyOnUseEffects`). Único gancho que sobra: `d20Roll` também lê `event.altKey` como vantagem (mjs ~4819) — o patch em `Actor.prototype.rollPericia` injeta `options.event = {...event, altKey:true}` quando o ator tem buff aplicável pra aquela perícia. Testes de ATAQUE nem passam por `rollPericia` (mecanismo separado, `Concentração de Combate`); Fortitude/Reflexos/Vontade são EXCLUÍDOS (`isEligibleSkill`) — no T20 vivem em `system.pericias` mas são "testes de resistência", não "testes de perícia".
- **Dois modos:** **"once"** (base — sem aprimoramento de escopo): buff de 1 uso, consumido no 1º teste de perícia elegível (`consumeAfter` deletado só se `orig.call(...)` não retornou `undefined` — dialog cancelado não consome) ou expira no início do próximo turno do alvo (`combatTurnChange`, `expireKind:"round"`, igual Concentração). **"persistent"** (+2 ou +5-grupo presente): dura a CENA inteira, sem consumo, mas restrito a um ESCOPO de atributo (`scopeAttrs`) — `attrInScope` compara `pericia.atributo` (`system.pericias[key].atributo`, resolvido de `CONFIG.T20.pericias[key].abl` na preparação) contra a lista.
...
- **⚠️ BUG v1.100.0→v1.100.1 (crítico, resolvido):** o item do compêndio chama-se só **"Orientação"** — "Divina" no pedido do usuário descrevia o TIPO da magia (`tipo:"div"`), não fazia parte do nome. `SPELL_NAME` estava `"orientacao divina"`, string que não existe em NENHUM nome real → a detecção do cast nunca disparava, com ou sem aprimoramentos (bug reportado como "sem aprimoramentos não faz nada", mas na real acontecia sempre). Fix: `SPELL_NAME = "orientacao"`. **Verificado ao vivo (Al Simmons, Arcanista 3):** base sem aprimoramento → card postado, AE `mode:"once"` no alvo, próximo teste de perícia saiu `2d20kh` (formula real capturada via console), buff consumido (`stillHasBuff:false`) após o uso; +2 PM (escolha "Força" no modal) → AE `mode:"persistent", scopeAttrs:["for"]`, Atletismo (For) saiu `2d20kh` e a AE PERMANECEU (não consome), Furtividade (Des, fora do escopo) saiu `1d20` normal.
