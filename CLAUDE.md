...
**⚠️ BUG v1.109.0→v1.110.0 (real, resolvido) — dano/acerto da Pata vazava bônus de OUTRAS armas equipadas** (relatado com Aller: uma Manopla Certeira equipada aplicava o bônus "Certeira" no ataque da Pata Inseto). Achado lendo `tormenta20.mjs` diretamente (`AbilityUseDialog.create` E o branch `configureDialog:false` de `Item.prototype.roll`): os DOIS caminhos nativos de `roll()` juntam `item.actor.effects.filter(ae => ae.getFlag("tormenta20","onuse") && ae.flags.tormenta20[tipo])` **sem checar `ae.origin === item.uuid`** — ao contrário da Manopla (que filtra por `origin` manualmente, `t20-fixes/manopla-upgrades.ts`). Como aprimoramentos de arma (ex.: "Certeira") são copiados pro `actor.effects` e ficam habilitados persistentemente (não são um checkbox por-rolagem), QUALQUER ataque do ator — inclusive o da Pata, que não tem relação nenhuma com a Manopla — os pegava. Fix (`rollPataAttackClean`): não chama `weapon.roll()` de jeito nenhum (nem com dialog nem sem) — chama `clone.rollAttack({options:{}})` + `clone.rollDamage({options})` + `clone.displayCard({options,...})` DIRETO no clone da Pata (os mesmos métodos internos que `Item.roll()` usa por baixo). Nenhum dos três lê `actor.effects` — só `itemData.rolls` (a fórmula fixa da Pata) + o `options` vazio que passamos — e `displayCard` ainda posta o MESMO chat card nativo (`flags.tormenta20.itemData` etc.) que `auto-damage` já sabe processar, então nada mais no fluxo (RD/reações/aplicação de dano no alvo T) precisou mudar.
