...
> **Como testar de forma confiável (não é regra, é técnica):** [`docs/live-testing-browser-gotchas.md`](docs/live-testing-browser-gotchas.md) — gotchas de automação de browser achados verificando ao vivo (múltiplos `DialogV2` sobrepostos e clique por `ref` acertando a janela errada, mismatch de coordenadas `read_page`/screenshot, técnica de contar chamadas de API de módulo terceiro sem adivinhar, fingerprint rápido de estado de ator pra restaurar depois).
...
src/
  ...
  anim-presets/index.ts        — Memória de animações de skills (Automated Animations): captura/aplica flags.autoanimations por magia/poder/arma (nome exato) + fallback por categoria de disparo (arco/besta/arma-fogo); prompt ao adicionar
  ...
  hud/mobile-mode.ts           — Modo mobile (Fase 1): setting hud.mobileMode (auto/mobile/desktop) + isMobileModeActive() puro + toggle + esconde chrome desktop
...
### "T20 Overhaul" — aba própria na sidebar (v1.8.0 → sidebar button v1.112.0 → aba nativa v1.113.0)

`src/ui/skills-menu.ts` é o REGISTRO puro (sem DOM) das ações de skills ativas + acesso aos compêndios do módulo. Cada sistema chama `registerSkillAction({ id, label, icon, color, isVisible(), onClick() })` em seu `setup*()` — API inalterada desde v1.8.0. `getVisibleActions()`/`getVisiblePacks()`/`executeSkillAction(id)`/`openCompendium(id)` são consumidos por `src/ui/T20OverhaulTab.ts`, que faz a exibição de verdade.

**v1.113.0 — de Dialog pra ABA NATIVA de verdade:** pedido do usuário — o botão consolidado (v1.112.0) abria um `Dialog` flutuante; ficou pedido que abrisse "a barra lateral como todos os botões do lado direito fazem". Fix: registrado no MESMO mecanismo nativo que Chat/Combate/Atores/Configurações usam (`Sidebar.TABS` + `CONFIG.ui`, lido direto de `client/applications/sidebar/{sidebar,sidebar-tab}.mjs` na instalação do Foundry — não documentado publicamente, mas é o padrão que TODA aba nativa usa).

- **Mecanismo** (`src/types/sidebar.d.ts` + `src/ui/T20OverhaulTab.ts`): `Sidebar.TABS` é um registro estático plain-object (`{tooltip, icon}` por id) que o template nativo `tabs.hbs` itera pra desenhar os ícones de `#sidebar-tabs` — extensível por módulos, igual `CONFIG.ui` (que instancia `ui[id] = new cls()` pra cada entrada, no `Game#initializeUI()`, ANTES do `ready`). `registerT20OverhaulTab()` escreve nos dois (`Sidebar.TABS["t20Overhaul"]` + `CONFIG.ui["t20Overhaul"] = T20OverhaulTab`) no hook `init` — MESMA exigência de timing do `CONFIG.ui.hotbar` (`registerFooterHud()`).
- **`T20OverhaulTab extends HandlebarsApplicationMixin(AbstractSidebarTab)`** — MESMO truque de `T20FooterHud extends Hotbar` (ver `hud/T20FooterHud.ts`): o mixin é necessário porque `ApplicationV2` pura tem `_renderHTML`/`_replaceHTML` abstratos (sem os dois a aba nem seria "renderable" — checado no próprio `application.mjs`), mas a classe NUNCA deixa o template real compilar — `PARTS` aponta pra `public/templates/t20-overhaul-tab.hbs` (arquivo mínimo, só existe pra o preload não dar 404) e `_renderHTML` é sobrescrito com string-builder puro (reusa `getVisibleActions()`/`getVisiblePacks()`).
- **Renderiza uma vez, não a cada clique de aba:** `Sidebar#_onFirstRender` chama `#renderTabs` pra TODAS as tabs (`Object.keys(TABS)`) logo no boot — trocar de aba só alterna uma classe CSS (`active-<tab>`), não re-renderiza. Por isso `refreshSkillsMenu()` (agora só `ui.t20Overhaul?.render()`) continua sendo chamada nos mesmos gatilhos de sempre (`registerSkillAction`, `renderSidebar`, `ready`, `canvasReady`) — sem isso o conteúdo ficaria estático desde o boot.
- **Clique num item NÃO fecha a aba** (diferença do Dialog antigo) — é um painel persistente agora, não um popup; após rodar a ação, `T20OverhaulTab` chama `this.render()` de novo (reflete mudança de estado, ex.: uma ação que sumiu).
- **Compêndios** inalterado: `pack.visible !== false` (`CompendiumCollection#visible`, mesma checagem de permissão da aba Compêndios nativa) filtra o que cada usuário vê; clique chama `pack.render(true)`.
- **Bônus de graça, não testado a fundo:** `AbstractSidebarTab` suporta pop-out (botão direito no ícone) igual as abas nativas — herdado automaticamente, não é código nosso.
- **Migração dos 3 botões órfãos (pedido do usuário — "faltou a migração"):** `treasure/index.ts` ("Gerar Tesouro"), `encounter-roller/index.ts` ("Encontros Aleatórios") e `hidden-test/index.ts` ("Solicitar Teste Secreto de Perícia") ainda injetavam botões avulsos em `menu#scene-controls-layers` (o modelo ANTIGO do skills-menu, pré-v1.8.0) — nunca tinham sido migrados pro registro compartilhado. Convertidos pra `registerSkillAction()` (GM-only via `isVisible:()=>!!game.user?.isGM`, mesmo `onClick` de sempre) — removida toda a lógica de injeção/DOM própria de cada um.
- **CSS:** `skills-menu.css` reescopado de `.window-app.t20-dialog` pra `.t20-overhaul-tab` (classe que `T20OverhaulTab.DEFAULT_OPTIONS.classes` adiciona ao elemento raiz da aba); tom dourado do ícone via `#sidebar-tabs [data-tab="t20Overhaul"] { color:#ffd86b }` (não dá mais pra setar inline, o botão é gerado pelo template nativo).
- **Verificado ao vivo (world arton, GM2):** aba aparece em `#sidebar-tabs` com o ícone dourado; clicar expande a sidebar mostrando "AÇÕES" (5 itens, incluindo os 3 migrados) + "COMPÊNDIOS" (6 packs); clicar num compêndio abre o browser SEM fechar a aba; clicar "Gerar Tesouro"/"Encontros Aleatórios"/"Solicitar Teste Secreto de Perícia" abre cada modal corretamente (Encontros preservou o contador do gatilho acumulado); trocar pra aba Chat esconde nosso painel, voltar mostra de novo — troca de aba nativa funcionando nos dois sentidos.
- **⚠️ Fix v1.113.0→v1.113.1 (achado ao vivo, texto mal formatado):** a coluna da sidebar é estreita e os rótulos originais dos compêndios (`"T20 Overhaul — Cruzado (Clérigo Variante)"`, `"T20 Overhaul — Poções e Pergaminhos"`, ...) quebravam em 2 linhas de forma feia, com alturas de botão inconsistentes entre linhas simples/duplas. Fix: `stripPackLabelPrefix` (`skills-menu.ts`, puro/testado) remove o prefixo `"T20 Overhaul — "` — redundante DENTRO da própria aba T20 Overhaul — só na EXIBIÇÃO desta lista (não mexe no metadata real do pack, que continua com o nome completo em Compêndios/instalação). `.skill-row` ganhou `min-height:40px`, `font-size` reduzido (0.95rem→0.85rem), `line-height:1.3` e `box-sizing:border-box` — todos os botões (ações + compêndios) ficam com altura uniforme e o texto cabe numa linha só na maioria dos casos.
...
- **Pipeline de compêndios**: fontes em `packs-src/<pack>/*.json` (1 doc/arquivo; `_key` obrigatório INCLUSIVE nos embedados: `!actors.items!<actorId>.<itemId>`, `!actors.items.effects!<aId>.<iId>.<eId>`, pastas `!folders!<id>`). `npm run build:packs` compila → `packs/` (gitignorado). CI (release.yml e beta-release.yml) compila e inclui no zip. module.json tem a entrada `"packs"`: `ameacas` (Actor, GM-only) e `weapon-properties` (JournalEntry "Propriedades de Arma", ownership OBSERVER p/ jogadores). Suporta qualquer tipo de doc (JournalEntry usa `_key:"!journal!<id>"` + páginas `!journal.pages!<journalId>.<pageId>`).
  - **`packFolders` (v1.113.2):** sem isso, os 6 packs do módulo caem cada um na pasta AUTOMÁTICA do seu tipo na aba nativa Compêndios (`Items`/`Actors`/`Journals`), misturados com dezenas de packs do sistema/outros módulos — achado ao vivo depois do usuário reportar "não tem compêndio de poções/pergaminhos" (o pack EXISTIA, `game.packs.get(...)` confirmava 1246 itens, só estava enterrado sem rótulo próprio). Fix: `module.json` ganhou `packFolders: [{name:"T20 Overhaul", sorting:"a", color:"#c9a76a", packs:[...os 6 names]}]` (schema real do core, `common/packages/base-package.mjs` — `packs` usa os `name` curtos do array `"packs"`, não os ids `t20-theme-overhaul.<name>`). **Não confundir com a aba custom "T20 Overhaul"** (`T20OverhaulTab`, v1.113.0) — são DUAS coisas paralelas: a aba condensa ações+atalhos; a pasta agrupa os packs na aba Compêndios NATIVA. `packFolders` é "inicializado uma vez por mundo" no SERVIDOR (`dist/packages/world.mjs`) — precisa do mundo (re)carregar pra criar a pasta; não verificado ao vivo nesta sessão (exigiria relançar o mundo, senha de admin que não devo digitar).
...
**Categorias de arma de disparo — arco/besta/arma de fogo (v1.114.0):** pedido do usuário: "observe as animações que adicionei aos tokens... faça com que as que adicionei para o traque, arcos e besta nas fichas sirvam para todos os arcos, armas de fogo e bestas". Até aqui a memória só cobria `magia`/`poder` por NOME EXATO — armas ficavam de fora inteiramente, e mesmo se cobertas, uma animação capturada em "Arco longo" nunca se aplicaria a "Arco Curto".

- **T20 não tem campo de categoria de arma** (arco/besta/arma de fogo) — só `system.proposito` genérico, `"disparo"` pra QUALQUER arma que atira (arco, besta, funda, zarabatana, arma de fogo de verdade). `classifyRangedWeapon(nome, proposito)` (puro/testado, `anim-presets/index.ts`) resolve por NOME: `proposito !== "disparo"` → `null` (arma corpo-a-corpo/arremesso, sem categoria — só nome exato, como sempre); contém "arco" → `"arco"`; contém "besta" → `"besta"`; senão → `"arma-fogo"` (catch-all — cobre armas de fogo de verdade E os casos avulsos como funda/zarabatana, decisão do usuário foi só 3 baldes). ⚠️ Confirmado ao vivo que "arcabuz" NÃO contém "arco" como substring (a-r-c-**a**-b-u-z, não a-r-c-**o**) — sem risco de falso-positivo.
- **Terceira camada de preset**: `AnimPresetLibrary.weaponCategoryPresets?: Record<WeaponAnimCategory, AnimPreset>` (`bundled-presets.ts`) + setting mundo `animWeaponCategoryPresets` (mesmo padrão merge world-vence-bundled de `getMergedPresets`). `resolvePresetForItem(item)`: tenta nome exato primeiro (`getPreset` — cobre armas ESPECÍFICAS capturadas por nome, igual magia/poder); se não achar E `item.type==="arma"`, cai pra `classifyRangedWeapon` + categoria. Retorna também `category` (não-null só quando veio do fallback) — o prompt anota "Sugerida pela categoria de arma: X (não é um preset específico para "Y")" quando aplicável, pra não confundir com um match exato.
- **3 presets bundled capturados ao vivo** de armas reais dos jogadores (idênticos entre bows testados — "Arco longo" e "Arco de Guerra Maciço" usam a MESMA animação `arrow/regular/regular`, confirmando que o usuário já padronizou por categoria manualmente): `arco` (seta), `besta` (virote, `bolt/physical/white`), `arma-fogo` (bala, `bullet/3/orange` — capturado de "Traque", um item homebrew do Al Simmons; `proficiencia` do item era `"simples"`, não `"fogo"` — reforça por que a classificação é por NOME/proposito, não por proficiência).
- **Captura/scan/prompt agora cobrem `arma`** (antes só `magia`/`poder`): `ANIMATABLE_TYPES = Set(["magia","poder","arma"])` centraliza o filtro de tipo em `captureActorAnimations`/`offerForItem`/`scanActorForOffers`/hook `createItem`. `isVisible` do botão "Animações: verificar ficha" agora também considera `weaponCategoryPresets` não-vazio.
- **Verificado ao vivo (world arton, GM2 impersonando Albus, que tem "Besta leve" SEM animação):** clicar "Animações: verificar ficha" → dialog "Animação Memorizada — Besta (categoria)" com a nota de categoria + status dos 3 módulos (✓sequencer ✓autoanimations ✗levels-3d-preview, aplicável mesmo assim); "Aplicar" grava exatamente a config capturada da "Besta leve" original (`bolt/physical/white`) — fallback de categoria funcionando fim-a-fim pra uma arma que o sistema nunca tinha visto antes.
- **Varredura completa + 5 presets exatos capturados (v1.114.1):** pedido do usuário — "verificou os outros itens na ficha? adicionei pra magias e armas como o ataque desarmado e membros extras". Sweep ao vivo em TODOS os atores (`magia`/`poder`/`arma` com `flags.autoanimations`) achou 4 gaps fora do que já estava no bundled: **Armadura Arcana** e **Explosão de Chamas** (magias), **Ataque desarmado** e **Pata Inseto 1/2 (Membros Extras)** (armas corpo-a-corpo — sem fallback de categoria, que só existe pra `proposito:"disparo"`). Capturados por NOME EXATO (mesmo mecanismo de sempre) e promovidos direto pro bundled. "Pata Inseto" é especialmente valioso bundled: é a MESMA arma que o `membros-extras/index.ts` do próprio módulo cria automaticamente pra QUALQUER personagem que ganhe o poder — a animação "claw" passa a se aplicar sozinha (via prompt) pra todo mundo que pegar Membros Extras, não só quem a capturou manualmente. "Ataque desarmado" é universal (todo personagem T20 tem o item nativo) — mesmo raciocínio.
- **Verificado ao vivo (Al Simmons, "Ataque desarmado" sem animação):** "Animações: verificar ficha" → dialog "Animação Memorizada — Ataque desarmado" **sem** a nota de categoria (match por nome exato, `category:null`) → Aplicar grava `unarmedstrike/darkred` idêntico ao capturado.

### Seta Infalível de Talude — divide setas entre alvos + animação por seta (v1.115.0)

`src/seta-infalivel/`. Magia Arcana 1 (Evocação): 2 setas de 1d4+1 de essência (aprimoramentos: +2 PM troca pra lanças 1d8+1 caindo do céu; +2/+4/+9 PM sobe pra 3/5/10 setas/lanças). "Você pode lançar as setas em alvos diferentes ou concentrá-las num mesmo alvo"; um bônus de dano de magias (ex.: Arcano de Batalha) "é aplicado em apenas uma seta".

- **Problema:** o T20 nativo trata a magia como single-target — com `resistSkill:null` (sem teste de resistência) e vários alvos marcados (`game.user.targets`), o fluxo genérico de `spell-resistance/index.ts` mandava o **dano TOTAL** (todas as setas somadas) pra CADA alvo igualmente, em vez de dividir.
- **Sem re-rolar nada:** `extractArrowResults(roll, dieFaces)` lê os resultados INDIVIDUAIS dos dados já rolados via `roll.dice.find(d=>d.faces===dieFaces).results` (mesmo padrão de `adamante/esoteric.ts:collectActiveOnes` — `Roll.dice` agrupa por faces independente de quantos termos separados compõem a fórmula). O bônus extra de classe é inferido por subtração: `computeExtraBonus(total, arrowResults, arrowCount) = total − Σarrows − arrowCount` (cada seta sempre carrega um "+1" fixo do texto da magia, então o que sobra é o bônus de fora).
- **Contagem/tipo de dado por aprimoramento** (`computeArrowCount`/`isLancaVariant`, regex em `flags.tormenta20.onUseEffects[].description`): "três"/"cinco"/"dez" → contagem; "surgem e caem do céu" (cláusula EXCLUSIVA do aprimoramento de lanças — ⚠️ não basta procurar "lança" na descrição, os aprimoramentos de CONTAGEM usam o texto genérico "setas/lanças" mesmo sem trocar o dado) → d8 em vez de d4.
- **Fluxo** (hook em `spell-resistance/index.ts::processSpellMessage`, logo ANTES do loop genérico de despacho, só quando `!isHeal && resistSkill===null`): `maybeHandleSetaInfalivel` detecta a magia por nome (`norm().includes("seta infalivel")`), calcula setas/bônus e:
  - **1 alvo só** → o total genérico já está certo (nada a dividir); só dispara a animação e retorna `handled:false`, deixando o loop genérico seguir normalmente.
  - **2+ alvos** → abre um `DialogV2` (`t20-seta-infalivel-dialog`) com 1 linha por seta (valor rolado, `<select>` de alvo pré-preenchido round-robin, e — se houver bônus extra — 1 rádio "qual seta carrega o bônus", default na 1ª) → no Confirmar, `groupArrowDamage` soma por alvo e despacha via um `dispatch(targetId, damageTotal)` injetado (reaproveita `dispatchToToken`, extraído do loop genérico pra um helper compartilhado — mesmo `SpellResistPreRollRequest`/socket de sempre). Cancelar aborta tudo (nenhum dano aplicado, aviso explícito) — mesmo padrão de "Aspirante a Herói".
- **Animação — 1x por seta:** `fireSetaAnimations` chama `AutomatedAnimations.playAnimation(sourceToken, item, {targets:[alvoDaSeta]})` uma vez por seta (staggered ~180ms, mesma API que `baforada/index.ts` usa), cada uma mirando o alvo REAL que a seta recebeu — inclusive no caso de 1 alvo só (N vezes pro mesmo alvo). ⚠️ **Ao contrário da Baforada, o cast nativo NÃO é cancelado** (o picker de aprimoramentos do T20 continua intacto) — a preocupação teórica era o Automated Animations disparar automaticamente por conta própria durante o cast nativo, ANTES do nosso disparo manual, duplicando a contagem.
- **✅ Verificado ao vivo (v1.115.0, world arton) — SEM overshoot:** testado com Al Simmons (magia real com animação configurada, `magicmissile`/roxo, JB2A) contra 2 alvos (Al'gazaha + Aller Brushfighter). Instrumentando `AutomatedAnimations.playAnimation` (wrap temporário contando chamadas): **exatamente 2 chamadas**, uma por seta, cada uma com o `targets` certo (Al'gazaha então Aller Brushfighter, ~195ms de intervalo — bate com o stagger de 180ms) — zero chamadas extras do A-A nativo antes ou depois. Console sem erros. Conclusão: o disparo automático nativo do A-A (quando existe) não passa pela MESMA API pública `playAnimation` que usamos — não há overshoot observável nesta integração; o risco documentado anteriormente está resolvido.
- **✅ Verificado ao vivo — divisão de dano correta em 2 cenários reais:**
  1. **Base, sem bônus** (Al'gazaha, 2 setas, sem aprimoramento): roll real `2d4 + 2` = dados [2,3], total 7. Dialog mostrou "Seta 1: 2+1=3 → Al Simmons" / "Seta 2: 3+1=4 → Aller Brushfighter" (default round-robin correto, sem coluna de bônus — `extraBonus=0` corretamente detectado). Aplicado: Al Simmons 42→39 PV (−3 exato); Aller Brushfighter RD Essência auto-detectada (3) reduziu o dano líquido pra 1 (4−3), consumindo 1 de PV temp (5→4) — confirma de quebra que a RD automática do modal de resistência funciona corretamente com o dano já dividido.
  2. **Com bônus de classe real** (Al Simmons, mesma magia, +9 de bônus de alguma fonte da própria ficha): roll real `2d4[essencia] + 2[essencia] + 9` = dados [4,4], total 19. `computeExtraBonus(19,[4,4],2) = 19−8−2 = 9` — bateu exatamente com o "+9" que o próprio T20 rolou. Dialog mostrou a coluna de bônus ("Bônus (+9)", rádio na Seta 1 por padrão) corretamente. Modais de aplicação resultantes: Al'gazaha (seta com o rádio) **14** (5+9), Aller Brushfighter (seta sem o rádio) **5** — exatamente os valores esperados.
- Todos os 3 atores usados no teste (Al'gazaha, Aller Brushfighter, Al Simmons) tiveram PV/PM capturados antes e restaurados aos valores EXATOS depois (regra de prioridade máxima); item temporário "Seta Infalível" criado em Al'gazaha pra o 1º teste foi deletado ao final (38 itens, igual ao original).

### ⚠️ BUG DE CI (v1.116.0→v1.116.4, resolvido) — Release falhava no `npm install`, CI não

3 tags seguidas (v1.116.0, v1.116.1, v1.116.3) tiveram o workflow **Release** falhando sempre no mesmo passo — `npm install` morrendo com `Cannot read properties of null (reading 'edgesOut')` — enquanto o workflow **CI** (mesmo commit, mesmo `npm install`) passava limpo toda vez. Causa: `release.yml` tinha `node-version: "22"` (bumpado numa sessão anterior só por causa do `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24` — necessário pras actions JS de terceiros tipo `softprops/action-gh-release`, NÃO pro próprio `npm install`) enquanto `ci.yml`/`beta-release.yml` usam `"20"`. **`package-lock.json` não é commitado neste repo** (`.gitignore`) — sem lockfile, `npm install` resolve a árvore de dependências do zero a cada run, e o Arborist do npm bundlado com Node 22 tem um bug conhecido nesse cenário que o do Node 20 não tem. Fix: `release.yml` voltou pra `node-version: "20"`. Como as 3 tags quebradas NUNCA publicaram artefato (falhava antes de chegar no zip), foram deletadas (`git tag -d` + `git push origin --delete`) e consolidadas numa tag só (v1.116.4) em vez de re-tagear 3× — nenhum usuário ficou sem update real, só o Release nunca tinha ficado verde antes. **Lição:** depois de ajustar Node/ferramenta de UM workflow, checar se os workflows IRMÃOS (`ci.yml`/`release.yml`/`beta-release.yml`) ficaram dessincronizados — e verificar o Release de CADA tag antes de seguir pra próxima feature, não só no final de uma sequência de vários pushes seguidos (essa sessão tageou 3× seguidas sem checar CI entre elas, e só percebeu a falha ao checar todas de uma vez no fim).

### 28 ameaças novas — Deserto da Perdição + Guilda de Ladrões de Thartan (v1.116.0)

Pedido do usuário via skill `/criar-ameaca`: 11 conceitos (alguns com vários NPCs cada — recepção, cozinha, garçonetes, bandidos, leiloeiros) totalizando 28 ameaças novas, todas geradas seguindo o **Manual de Criação de Ameaças** (ND → tabela → sem cálculo, ver `.claude/skills/criar-ameaca/SKILL.md`).

- **`scripts/gen-ameacas-thartan.mjs`** (committed, reexecutável — **NÃO editar os `.json` de saída à mão**, ajustar o spec no script e rodar `node scripts/gen-ameacas-thartan.mjs && npm run build:packs`): gera as 28 ameaças + a pasta nova "Guilda de Ladrões de Thartan" em `packs-src/ameacas/`. Helpers reutilizáveis (`buildPericias`, `buildAtributos`, `buildTracos`, `buildPrototypeToken`, `weaponItem`, `powerItem`, `buildActor`) montam o boilerplate completo do schema (30 chaves de perícias, `tracos.resistencias` com as 13 chaves de tipo de dano, `modificadores`, `resources`, `prototypeToken`) a partir de specs compactos — mesmo padrão de `gen-divindades.mjs`/`gen-pocoes-pergaminhos.mjs`, só que pra NPCs de ameaça em vez de itens.
- **Ataque/dano LITERAIS na fórmula, não via referência de perícia:** o item "Raio de Gema" da Efira Silvertongue (referência) usa `parts:[["1d20","",""],["pont","",""],["0","",""]]` — o meio referencia a CHAVE da perícia `pont`, cujo total real depende de como o sistema computa perícias pra atores `npc`. Investigar esse formulário ao vivo não valia o risco pra 28 fichas de uma vez: os geradores usam `parts:[["1d20","",""],["<Atq literal>","",""],["0","",""]]` — a rolagem de ataque sempre sai `1d20+<bônus exato da tabela>`, sem depender de nenhum campo de perícia (dano já era literal desde sempre, ex. `"2d6+7"`). `detalhes.ataquescac`/`ataquesad` (texto de exibição) ficam sincronizados com o item de verdade — ao contrário da Efira, onde os dois campos divergem (achado ao ler o exemplo — provavelmente autoria manual não 100% reconciliada).
- **Organização das pastas do compêndio:** reaproveita pastas EXISTENTES do compêndio `ameacas` quando fazem sentido temático — `Custom Deserto` (`pr9E79ep3uamweOv`, já tinha só a Pantera do Vidro) ganhou o Sarim; `Custom Made` (`X9TuFXCvny1ptG5G`, catch-all genérico) ganhou Comandante Purista/Astralos/Elfarion/Vaskrith; `Ahlen` (`FMIEwltYudrY66ci`) ganhou a Jacelin (recepcionista da guilda de aventureiros da MESMA área nobre de Ahlen dos NPCs já existentes). Pasta NOVA `Guilda de Ladrões de Thartan` criada só pra agrupar os 22 NPCs ligados à guilda (líder, trabalhadores, atendimento, bandidos, funcionários).
- **Conceitos por item do pedido do usuário:**
  1. **Sarim Punho-do-Sol** — Humano 2,2m, Místico do Deserto da Perdição, ND8. ⚠️ **Corrigido pelo usuário em tempo real**: Místico não é monge/lutador marcial — é um "mago elemental" que controla diretamente elementos (fogo/terra, afinidade com o deserto) e pode alterar o tipo de dano das próprias magias à vontade. Por isso o papel virou **Especial** (não Solo) — tabela 2-3C, não 2-3A.
  2. **Comandante Herwin Aço-Puro** — Comandante Purista (facção antimagia de Arton), ND5, lutador corpo a corpo com magias equivalentes a conjurador nv5. Papel **Solo**.
  3. **Jacelin** — Recepcionista da guilda de aventureiros da área nobre de Ahlen, ND2, especialista em Furtividade/Enganação/Intuição, papel **Especial** (baixo combate, alta perícia).
  4. **Kara Thur** — Líder da Guilda de Ladrões de Thartan, assassina humana, ND12, papel **Solo** (Ataque Furtivo, Veneno Paralisante, Golpe Fatal).
  5. **Astralos** — Arquimago humano ND15, papel **Especial**.
  6. **Elfarion** — Arqueiro arcano elfo ND10, papel **Solo** (ataque à distância híbrido com magia).
  7. **Trabalhadores da guilda** (ND6, **Lacaio**, todos): Azathot (ferreiro Sulfure, imune a fogo), Megarah Al'Jaffar (artífice meio-elfa, engenhocas), Alef o Canhoto (conjurador humano trapaceiro, magia pela mão esquerda).
  8. **Atendimento** (ND1, **Lacaio**, quase sem combate — PV 9): Milla/Alphonse (recepção), Liana (barista), Falira/Aika/Valk (garçonetes humanas), Albur (garçonete elfa) — cada uma com 1 habilidade passiva de sabor (nunca provoca ataque de oportunidade, cura com bebida, etc.).
  9. **Bandidos** (NDs variados, **Lacaio**, raças diferentes por pedido explícito): Dhoreon (anão manco — deslocamento Lento 6m como personalização de conceito, não mexe na tabela), Arabella (hinne, Tamanho Pequeno), Marian (**golem de bronze** — `tipo:"con"`, não humanoide, imune a veneno/doença/sono/medo), Diomedes (trog).
  10. **Funcionários** (ND3, **Lacaio**): Jarif/Karin/Almanfi (cozinha), Sadira (moreau gato, depósito), Fillion (hobgoblin, depósito/segurança), Silvaril (dahlan) e Augustus (humano) — os 2 apresentadores de leilão, por pedido explícito ("devem ser belíssimos"), têm Car 5 (excepcional) e **+10** em Diplomacia/Enganação-Atuação.
  11. **Vaskrith** — Dragão jovem infectado pela Tormenta, ND5, papel **Solo**, com as 4 habilidades Lefeu **copiadas literalmente do texto do usuário** (Imunidades — lista de 12 itens; Insanidade da Tormenta — Vontade CD do ND ou perde PM; Percepção Temporal — nota que Sab já está somada em ataque/Defesa/Reflexos, "já contabilizado na ficha"; Visão Ampla — +5 Percepção, nunca flanqueado). `tipo:"mon"`.
- **Arte dos tokens — 29/31 empacotadas (v1.116.3):** o usuário gerou as artes fora do fluxo do Gemini/browser (resolveu por conta própria) e salvou 29 `.jpg` soltos em `Data/assets/a/` com nomes curtos e informais (`Herwin.jpg`, `Jariff.jpg` — note o typo de dois "f", intencionalmente mapeado pro `Jarif` correto —, `Alef o Canhoto.jpg` sem vírgula, etc.), fora do padrão de nome completo usado nos `img` das fichas. Copiadas (não movidas) pra `public/assets/{Thartan,Deserto da Perdição,Avulsos,Ahlen}/<Nome canônico da ficha>.jpg` — o gerador trocou TODOS os `img`/`texture.src` de `.png` pra `.jpg` (todas as artes entregues até agora vieram nesse formato). Faltam só **Aslan Vesper** e **Sanaa Vesper** (adicionadas depois, sem arte ainda — path `.jpg` pré-configurado por consistência, ajustar se vier outro formato).
- **Deploy:** mudou `packs-src/ameacas/` (dados) + `public/assets/` (imagens, ~47MB) — nenhum código `src/` novo, `npm run typecheck && npm test && npm run build` continuam de controle (nada quebrou, só confirma que o resto do bundle está intacto).
- **29ª ameaça — Dafodil (v1.116.1):** pedido de acréscimo do usuário — "exata mesma coisa que Silvaril só que mulher" (mesmo cargo/raça Dahlan/ND3/estatísticas/habilidade "Presença Magnética", só nome/gênero/arte mudam; arte já pronta, feita pelo próprio usuário). Adicionada no **FIM** do array `NPCS` do gerador de propósito — o script usa RNG SEEDADO determinístico pra gerar ids/nomes de arquivo, então inserir no meio teria mudado os ids de TODAS as ameaças seguintes já commitadas/tageadas em v1.116.0; no fim, a sequência de chamadas de `makeId()` das 28 anteriores fica bit-a-bit idêntica (confirmado via `git status` mostrando só o arquivo novo depois de re-rodar). De quebra, `now` (timestamp de `_stats`) virou uma constante fixa em vez de `Date.now()` — evita diff de timestamp em toda ameaça não-alterada a cada re-execução do gerador.
- **30ª/31ª ameaças — Aslan Vesper e Sanaa Vesper (v1.116.2):** dupla ligada por família (irmãos ou casal, texto deixa ambíguo de propósito). Aslan (Guerreiro, ND8, **Solo**: Fúria de Batalha, Guarda Inabalável, Golpe Giratório) e Sanaa (Feiticeira, ND7, **Especial**, conjuração por Carisma: `mist` com `atributo` sobrescrito pra `"car"` em vez do default `"int"` do skeleton — confirma que `buildPericias` aceita overrides parciais por perícia, não só `outros`/`treinado`). **Vínculo de Sangue** (ambos): +2 no ataque enquanto lutam lado a lado — habilidade de sabor que referencia o NOME do outro NPC na descrição, mesmo padrão narrativo de duplas ligadas. Adicionadas no fim do array (mesmo motivo de Dafodil).
- **⚠️ Cópia do pack durante o mundo rodando — quase-incidente evitado:** ao copiar `packs/ameacas` pro `Data/modules/.../packs/` em v1.116.0, o mundo `arton` estava ATIVO (LevelDB travado, `rm -rf` falhou parcial com "Device or resource busy" em `000028.ldb`/`LOCK`/etc.). Como o `rm` NÃO limpou o diretório de destino, o `cp -r` subsequente criou uma subpasta ANINHADA (`packs/ameacas/ameacas/`) em vez de sobrescrever — resultado: o pack ANTIGO ficou intacto (sem corrupção, ao contrário do incidente documentado em v1.104.2), só sobrou uma pasta extra inofensiva (removida manualmente depois). **Lição reforçada:** o teste "tentar apagar 1 arquivo, ver se dá busy" precisa vir ANTES de qualquer `cp`, não só antes do `rm` — cópia de pacote pra AppData com o mundo rodando fica pendente até o usuário reiniciar o mundo (ainda pendente nesta sessão).
- **Corrupção real do compêndio + recuperação (entre v1.116.3 e v1.117.0):** um `rm -rf`/`cp -r` interrompido enquanto o mundo estava rodando deixou o diretório `packs/ameacas` da instalação local num estado misto; um restart subsequente do mundo disparou a recuperação nativa do LevelDB, que descartou o manifesto válido e esvaziou o compêndio ("as ameaças sumiram"). Nada foi perdido na FONTE (`packs-src/` git-tracked + `packs/` recompilado localmente tinham tudo); recuperado com o mundo genuinamente parado (confirmado tentando apagar o `LOCK` — sucesso = seguro) via delete-e-copia atômica do diretório inteiro + `cmp -s` byte-a-byte nos 6 arquivos do LevelDB + smoke-test de nomes de ator no `.ldb` binário antes de autorizar o relaunch. Reforça a lição já documentada em v1.104.2: nunca mexer num pack LevelDB com o processo do Foundry potencialmente aberto.

### Mana (PM) + conjuradores com lista de magias reais nas ameaças (v1.117.0)

Pedido do usuário: "ajuste as ameaças para que elas tenham mana e suas habilidades custem mana quando necessário, e ajuste para que os casters tenham seus níveis de conjurador especificado e quais magias podem lançar" — aplicado às 31 ameaças de Deserto da Perdição/Thartan/Vesper (v1.116.0-1.116.4), pesquisando a convenção real "nos módulos ameaças" (bestiário do sistema) em vez de inventar do zero.

- **Convenção de conjurador copiada de "Chiisaku, o Ogro Mago"** (ameaça pré-existente do bestiário): bio termina com `"Magias — Como um mago de 3º nível (CD 17)."` seguido de lista com marcadores das magias — texto puramente descritivo/de bookkeeping pro mestre, sem mecânica própria. Replicado nas 6 ameaças casters (Sarim, Herwin, Astralos, Elfarion, Alef, Sanaa Vesper).
- **Magias embutidas como itens REAIS**, não poderes reinterpretados — convenção descoberta lendo "Sílfide Feiticeira" (outra ameaça pré-existente): cópias literais de `type:"magia"` do compêndio `tormenta20.magias`, com `ativacao.custo`/`circulo`/`escola`/`tipo`/`resistencia`/`rolls`/`effects` (aprimoramentos onuse) intactos — não magias inventadas. `spellItem(actorId, key)` (novo helper em `scripts/gen-ameacas-thartan.mjs`) lê de `scripts/data/ameacas-spells.json` (13 magias reais, colhidas ao vivo via `game.packs.get("tormenta20.magias").getDocuments()` + filtro temático + checagem manual de `tipo` arc/div/uni compatível com o atributo de conjuração de cada NPC e círculo dentro do nível declarado).
- **PM formulaico pros ~25 não-casters:** `pmCostForNd(nd) = max(1, round(nd/2))` (custo por habilidade ativa) e `pmPoolForNd(nd) = max(6, nd*3)` (pool base) — derivados diretamente da diretriz do próprio manual de criação de ameaças (`.claude/skills/criar-ameaca/SKILL.md`: "evite... 3 PM por ND"). Habilidades que eram "Padrão, 1×/cena" (grátis) viraram "Padrão, N PM" (debitado automático pelo `automaticManaSpend` nativo, via `ativacao.custo`). Marian (golem de bronze, Int -4, sem mente) ficou **sem `pmMax`** de propósito — construto não tem força de vontade/mana, decisão de design documentada inline.
- **6 casters com pool maior e custom** (15-40 PM, não a fórmula genérica — ancorado numa escala observada em bestiário de PM alto real como Ravarimm/Al-khab, sem fidelidade estrita a uma fórmula oficial, que também não existe pra esses casos): Sarim Punho-do-Sol (Místico ND8 — reconceituado de "Solo" corpo-a-corpo pra **Especial**/conjurador de elementos após correção do usuário: "místico é uma espécie de mago elemental"), Comandante Herwin Aço-Puro, Astralos, Elfarion, Alef o Canhoto, Sanaa Vesper (Feiticeira ND7, conjuração por Carisma).
- **`spellItem` — bug de compilação achado só no `build:packs` (real, resolvido):** o helper copiava os `effects` (aprimoramentos onuse da magia) com `_id` novo mas **sem `_key`** — o `compilePack` do `@foundryvtt/foundryvtt-cli` (LevelDB) exige `_key` em TODO documento embutido, inclusive efeitos dentro de itens dentro de atores (`!actors.items.effects!<actorId>.<itemId>.<effectId>`, convenção já documentada acima em "Pipeline de compêndios"). Sem isso, `ClassicLevel._checkKey` lança `LEVEL_INVALID_KEY` ("Key cannot be null or undefined") na hora de compilar — só aparece no BUILD, não no `node scripts/gen-ameacas-thartan.mjs` (que só escreve os JSON-fonte, não valida contra o schema do LevelDB). Fix: `spellItem` agora gera `_key` pra cada effect junto do `_id`.
- **Trocas de imagem pós-deploy + wiring (v1.117.0):** o usuário substituiu manualmente os 29 arquivos `.jpg` iniciais por versões `.png` (mesmos nomes, com 2 exceções de rename: `Sarim Punho-do-Sol.jpg` → `Sarim, Punho Solar.png`; `Alef, o Canhoto.jpg` → `Alef, O Canhoto.png`) direto nas pastas do módulo, e informou que as imagens de Aslan/Sanaa Vesper (nunca copiadas antes — não existiam quando os 29 primeiros arquivos foram entregues) estavam esperando em `Data/assets/a/`. Ajustado: as duas Vesper copiadas pra `public/assets/Avulsos/`, e todos os 30 campos `img`/`texture.src` do gerador (extensão `.jpg`→`.png` + os 2 renames) atualizados para casar exatamente com os arquivos reais em disco (checado 1:1 antes de regenerar).
- **⚠️ Editar itens de uma ameaça JÁ SHIPPED desloca o RNG de TODAS as ameaças seguintes no array — achado real, não coberto pela regra "adicionar no fim" já documentada:** a regra de "sempre appendar NPCs novos no FIM do array" (ver Dafodil/Vesper acima) só protege contra perturbar ids ao ADICIONAR uma ameaça nova — não cobre EDITAR os itens de uma ameaça já existente NO MEIO do array (ex.: adicionar 3 itens de magia no Sarim, que fica bem no início do bloco Deserto/Thartan). Cada `makeId()` consumido a mais por um item novo desloca a sequência determinística de TODOS os `_id` gerados depois dele — resultado: regenerar produziu arquivos com hash NOVO pra quase todas as 31 ameaças (Sarim manteve o dele, por coincidência de ser o próprio editado — seu id de ATOR já tinha sido consumido antes dos itens), deixando pra trás os arquivos ANTIGOS ainda rastreados pelo git como duplicatas órfãs. Resolvido removendo (`git rm`) os 30 arquivos antigos superados pelos novos; aceito como churn de ID inevitável (e sem custo real: o pack é PLAYER:NONE, recompilado do zero a cada build, sem estado de jogador dependente desses ids específicos).

### Mia, Grom, Cedric — 3 ameaças avulsas via /criar-ameaca (v1.118.0)

Pedido do usuário: 3 ameaças novas sem tema em comum (batedora dos bosques, lenhador/guarda, vigia de milícia) — geradas via skill `/criar-ameaca` seguindo as tabelas 2-3 A/B do manual estritamente (sem PM formulaico inventado, direto das colunas ND/papel). Adicionadas no **FIM** do array `NPCS` de `scripts/gen-ameacas-thartan.mjs` (mesmo motivo de Dafodil/Vesper — não perturbar ids já deployados), pasta `FOLDER_CUSTOM_MADE` (catch-all, mesmo destino de Herwin/Astralos/Elfarion/Vaskrith/Vesper).

- **Mia** (Moreau Felina, Batedora dos Bosques, ND2, **Solo** — tabela 2-3A): Reflexos forte(13)/Fortitude média(7)/Vontade fraca(2), Arco Curto `1d6+15` (avg 18.5 ≈ Dano 18 da tabela — offset +0.5 é o padrão já usado em todo o arquivo, dado par nunca bate em número inteiro exato). Emboscada Silenciosa (+1d6 vs Desprevenido/surpreso) + Salto Felino (1 PM).
- **Grom** (Moreau Urso, Lenhador e Guarda de Carga, ND3, **Solo**): Fortitude forte(15)/Reflexos média(9)/Vontade fraca(3), Machado de Lenhador `1d12+15` (avg 21.5 ≈ Dano 21). Golpe Derrubador (derruba, Fort CD 17, 2 PM) + Couro Espesso (RD 3 físico).
- **Cedric** (Elfo, Vigia da Milícia, ND1, **Lacaio** — asseclas de patrulha, cabem em grupo): Reflexos forte(10)/Fortitude média(5)/Vontade fraca(1), Espada da Milícia `1d8+13` (avg 17.5 ≈ Dano 17). Visão Élfica (passiva, imune a sono +2 vs Encantamento) + Grito de Alarme (1 PM, anti-surpresa em grupo).
- **PM**: os 3 seguem a fórmula genérica já documentada acima (`pmPoolForNd`/`pmCostForNd`), nenhum é caster — sem `spellItem`/lista de magias.
- **Artes**: fonte `Data/assets/NPCs/token_14.png` (Mia), `token_15.png` (Grom), `token_7.png` (Cedric) — nomes de arquivo genéricos do gerador externo do usuário, copiados pra `public/assets/Avulsos/<Nome>.png` com o nome canônico do ator (mesma convenção de renome já usada pros 29+2 arquivos anteriores).
- Verificado antes do build: `git status` confirmou só 3 arquivos novos em `packs-src/ameacas/` (nenhum id existente mudou), scan de `_id`/`_key` limpo, e Def/PV/CD/saves batendo exatamente com as colunas da tabela 2-3A/B pro ND/papel de cada um.

### Gem do Gemini pra retrato de ameaça — texto-para-imagem bloqueado nesta conta (v1.116.0)

Pedido do usuário: criar um Gem no Gemini (aba logada no browser do Claude app) pra gerar a arte circular de token no MESMO estilo já usado no compêndio (pasta Ahlen: Bolton Al'Gant, Edara Silvertongue, Flint — medalhão circular com borda em gradiente "fogo", pintura digital semi-realista estilo card art de RPG, fundo transparente fora do círculo).

- **Gem criado**: "T20 Overhaul — Retrato de Ameaça" (conta Gemini do usuário), com instruções detalhadas de moldura/estilo/conteúdo (ver histórico da conversa — não versionado no repo, vive só na conta Gemini).
- **⚠️ Achado real (não é bug do Gem, é limitação da ferramenta "Criar imagens" desta conta/versão do Gemini):** testado de 5 formas diferentes (Gem novo, Gem pré-existente "Token Generator", chat comum, templates prontos tipo "Lutador mítico") — **todo texto-para-imagem PURO é recusado** ("Sou um modelo de linguagem e não consigo ajudar com isso" / variações). Causa raiz: a ferramenta "Criar imagens" desta conta roda sobre o **Nano Banana 2**, que é um modelo de EDIÇÃO/transformação de imagem — ele exige uma foto/imagem de BASE enviada por upload e só a modifica; não gera do zero só por texto. Confirmado comparando com o Gem "Token Generator" (pré-existente, funciona) — ele SEMPRE exige uma imagem anexada como referência ("Use the uploaded image as a STRICT visual reference"), nunca texto puro.
- **Bloqueio de automação**: a ferramenta de browser deste projeto (`mcp__Claude_Browser__*`) não controla o seletor de arquivo NATIVO do Windows — não dá pra fazer upload de um PNG local pelo fluxo automatizado.
- **Decisão (usuário)**: eu escrevo os prompts de texto pra cada personagem (estilo já estabelecido, só a descrição do personagem varia) e o USUÁRIO faz upload de uma imagem-base + cola o prompt manualmente no Gem, gera as 28 artes, salva nos caminhos combinados e avisa quando terminar — eu integro depois (só trocar os 28 `img`/`texture.src` já apontados nos ameaças + rebuild dos packs).
...
### Modo mobile da HUD — Fase 1: fundação (v1.105.0, fix de verificação ao vivo v1.105.1)

`src/hud/mobile-mode.ts`. Pedido do usuário: jogador acessar ficha e rolar dados pelo celular. Decisão de arquitetura (avaliada em conversa antes de implementar): **não é um app separado** — é o MESMO Foundry, mesma sessão/URL/login, só uma UI diferente acima/abaixo de certa largura, porque o Foundry já roda inteiro num navegador com WebSocket em tempo real e o Footer HUD (v1.95.0+) já tem quase tudo que uma ficha compacta de celular precisaria (retrato, orbes PV/PM, grid de perícias, painel Inventário/Poderes/Magias, indicador de combate). Caminho alternativo (app nativo com API própria) foi descartado por exigir um serviço novo inteiro fora do Foundry, sem API pública estável pra rolagens — ver decisão registrada na conversa, não documentada em código.

- **Setting `hud.mobileMode`** (`client`, `config:false`, default `"auto"`): `"auto"` decide pela largura da viewport (`window.innerWidth < MOBILE_BREAKPOINT_PX`, **600px** — mesmo corte já usado no container query do modal de resistência `.smf-body`); `"mobile"`/`"desktop"` FORÇAM o modo independente da largura real. `isMobileModeActive(width, setting)` é pura/testável — o modo explícito sempre vence sobre a largura detectada.
- **Por que um toggle explícito, não só auto-detecção:** a flag "site para computador" do navegador mobile muda a largura reportada pra valores desktop (tipicamente ~980-1080px), o que já dá um jeito manual de escapar pro Foundry normal — mas só nessa direção. O toggle cobre o inverso (forçar mobile num tablet que se comporta como desktop) e serve pro GM testar/demonstrar sem precisar encolher a janela de verdade.
- **Toggle na própria HUD** (não no menu de configurações do Foundry — ruim de navegar no celular): botão pequeno (`.t20-hud-mobile-toggle`) na mesma linha do título "Perícias"/barra de carga, ícone muda por modo (`fa-arrows-rotate`/`fa-mobile-screen-button`/`fa-desktop`). Clique = `cycleMobileModeSetting()` (auto→mobile→desktop→auto, persiste) → reaplica a classe → re-renderiza.
- **Reatividade:** `wireMobileModeReactivity()` (chamado 1x em `setupFooterHud()`) aplica `document.documentElement.classList.toggle("t20-mobile-mode", ...)` na carga inicial e a cada `resize` de janela — mede `window.innerWidth` (a tela real do dispositivo), não a largura do elemento da HUD (que é exatamente o que o modo mobile está prestes a mudar).
- **CSS desta fase** (`hud.css`): com `.t20-mobile-mode` no `<html>`, esconde o chrome desktop do Foundry (`#board` canvas, `#sidebar`, `#scene-controls`, `#players`, `#scene-navigation-active`) e remove o cap de largura da HUD (`.t20-hud-root{width:100%}`, era `min(680px,100%)`).
- **⚠️ Escopo desta fase, de propósito:** a HUD ainda NÃO se reestrutura verticalmente pra ocupar a tela toda — ela continua ancorada como rodapé (`align-items:flex-end` em `#hotbar`), só mais larga e sem o canvas atrás. Empilhar retrato/orbes no topo + perícias OU painel direito como abas únicas de tela cheia (Perícias virando uma 5ª aba ao lado de Inventário/Poderes/Magias) é a **Fase 2 (M5)**.
- **⚠️ BUG v1.105.0→v1.105.1 (real, resolvido) — `#navigation` não existe nesta versão do Foundry:** a barra de navegação de cenas tem id **`#scene-navigation-active`** em v13.351 (`#navigation` não existe no DOM — confirmado ao vivo via `document.getElementById`), então a regra original era um seletor morto: o nome da cena ("🗡️ Thartan") continuava visível no canto superior esquerdo mesmo com `.t20-mobile-mode` ativo. Trocado o seletor; confirmado ao vivo que some junto com o resto do chrome.
- **⚠️ Botão do toggle pequeno demais pra achar (v1.105.1):** usuário reportou "não encontrei o botão". Verificado ao vivo (sessão real do usuário, GM2/Al Simmons, resize real do browser pra 375×812): o botão EXISTIA e FUNCIONAVA (clique cicla o modo, auto-detecção por resize também funciona — confirmado via `document.documentElement.className`), mas 20×20px espremido entre o título "Perícias", a barra de carga e o stepper de linhas era genuinamente fácil de não notar. Aumentado pra 26×26px com borda dourada (`var(--t20-accent)`, 1.5px) pra se distinguir visualmente dos botões do stepper (que usam a borda neutra `--t20-btn-border`) — confirmado ao vivo que fica claramente mais visível na mesma posição.
- **Verificado ao vivo (world arton, GM2, resize real do browser):** toggle cicla auto→mobile→desktop→auto (tooltip muda: "Automático"/"Modo celular forçado"/"Modo desktop forçado"); resize real da janela pra 375px ativa `.t20-mobile-mode` sozinho (sem clique) e esconde canvas/sidebar/scene-controls/players/scene-navigation; resize de volta pra desktop restaura tudo. **Gotcha à parte:** o próprio Foundry mostra um aviso nativo ("requer janela ≥1024×768") ao tentar ENTRAR na sessão (tela de login) com a janela já pequena — não é bug nosso, é validação nativa do core; não impede nada depois de já estar dentro da sessão (resize pra baixo funciona liso).

### Modo mobile da HUD — Fase 2: layout vertical de tela cheia (v1.106.0)

Empilhamento vertical prometido na Fase 1: cabeçalho (retrato+nome+classes+carga+orbes compactos) → banner "Sua vez" (se ativo) → conteúdo de UMA aba por vez → barra de abas embaixo. **Perícias vira uma aba a mais** ao lado de Inventário/Poderes/Magias/Macros (`MobileTabKey = RightTabKey | "pericias"`, `state.ts`) — no desktop elas ficam lado a lado; não cabe lado a lado numa tela de celular.

- **Reaproveitamento máximo**: `T20FooterHud._renderHTML` escolhe entre `buildFooterHudHtml` (desktop) e `buildMobileFooterHudHtml` (novo) via `isMobileModeElementActive()` (lê a classe já aplicada no `<html>`, não recalcula). O markup mobile é OUTRO arranjo, mas reusa os MESMOS builders/handlers: `buildOrbHtml`/`wireOrbInteractions` (orbes, sem mudança — a query é genérica em `root`), `buildSlotGridHtml` (grid+paginação), o clique em `[data-skill-key]`/`[data-item-id]` (já genérico), o drag-reorder por `[data-drag-key]` (já genérico, persistência por `actorId+listKey` continua compartilhada entre desktop e mobile pro MESMO listKey). Só a ARRANJAÇÃO do HTML e o estado de aba/página mobile (`getMobileTab`/`getMobilePage`, independentes de `rightTab`/`rightPage` do desktop — trocar de aba num modo não mexe no outro) são novos.
- **Rows fixo no mobile** (`MOBILE_ROWS = MAX_ROWS = 4`) — o stepper de linhas some (não faz sentido numa tela que já usa a altura toda); cálculo dinâmico de linhas pela altura real disponível (paralelo ao `colsForWidth` já existente pra colunas) fica de fora desta fase — ver "não implementado" abaixo.
- **`position: fixed; inset:0; height:100dvh`** no `#hotbar` em vez de tentar crescer dentro do fluxo nativo — medido ao vivo que `#ui-middle` divide `#ui-top`/`#ui-bottom` exatamente 50/50 via flex; forçar altura ali vazaria/cortaria de formas imprevisíveis. Overlay próprio cobre a tela inteira e a HUD deixa de ser um rodapé pra virar a UI principal.
- **Banner "Sua vez"**: mesmo `.t20-hud-next-turn` do desktop, só que com CSS mobile-scoped resetando pra fluxo normal (`position:static`, largura cheia) em vez do meio-círculo flutuante ancorado em `.t20-hud-panel` (que não existe neste layout).
- **⚠️ Trade-off aceito**: sem chat/log visível em modo mobile nesta fase (`#sidebar` continua escondido da Fase 1, e `#chat-notifications` também foi escondido — ver abaixo). O jogador rola e vê o resultado no PRÓPRIO Foundry se sair do modo mobile, ou (melhor) numa Fase 3 que dê alguma visão de chat compacta. Documentado como gap conhecido, não escondido.

**⚠️ Três bugs reais de stacking/CSS achados só ao vivo (nenhum detectável por teste unitário ou leitura de código) — todos em `hud.css`:**

1. **`#ui-middle` tem `z-index:1000` no CSS nativo `.mobile-improv`** (a classe que o PRÓPRIO Foundry adiciona ao `<body>` em telas estreitas — descoberta nova: o Foundry v13 JÁ TEM um sistema de UI mobile nativo completo, com `.mobile-navigation` (bottom bar própria, z:1002), drawers (`.window-selector`/`.mobile-menu`, z:1000) e tema `.themed.theme-dark/light`). Como `position:fixed` NÃO escapa o stacking context de um ancestral posicionado+com z-index, `#hotbar` ficava PRESO acima de QUALQUER diálogo nativo (`AbilityUseDialog` etc., ~103-110) não importa que z-index a gente desse a ele — bug real: clicar numa perícia "não fazia nada" (o diálogo abria, só que atrás da HUD, 100% inacessível, sem erro no console). Fix: `html.t20-mobile-mode #ui-middle { z-index:auto !important; }`.
2. **Zerar o z-index não bastou — `#ui-middle` também tem `transform: matrix(1,0,0,1,0,0)`** (identidade, visualmente um no-op) no CSS nativo. Por spec CSS, QUALQUER `transform` diferente da palavra-chave `none` cria stacking context, mesmo uma matriz identidade — continuava prendendo `#hotbar` atrás de painéis de terceiros ancorados direto em `#interface`. Fix: `transform:none !important` junto do z-index no mesmo seletor.
3. **As classes nativas `md`/`sm`/`min`** (mantidas de propósito em `_onResize()` desde a Fase 0 do projeto original, porque outro código nativo depende delas existirem) trazem consigo o comportamento responsivo NATIVO do Foundry — incluindo um `transform: translateY(850px)` que empurra a hotbar pra FORA da tela (o core esconde a hotbar nativa em telas estreitas, já que ele mesmo não a usa nesse modo — usa os drawers). Resultado: tela inteira preta, nada renderizado, sem erro. Fix: `transform:none !important` no PRÓPRIO `#hotbar` também (incondicional, não só na classe `.offset` que já existia pra outro caso).
4. **Um z-index baixo era necessário sim, mas ESTREITO**: sem nenhum, o painel de status ativo de um módulo de terceiros (`.visual-active-effects`, community module "Visual Active Effects", z-index:100, filho direto de `#interface`) cobria o cabeçalho de forma inconsistente/flakeando entre renders (mesma camada de empate, ordem do DOM decidindo). `z-index:101` no `#hotbar` (depois dos fixes 1+2 acima, que tiraram `#ui-middle` do caminho) vence esse painel (100) e ainda perde pros diálogos nativos (103+, confirmado em 3 aberturas diferentes) — intervalo deliberadamente estreito, não uma tentativa de "ficar por cima de tudo".

**Não implementado / gaps conhecidos desta fase:**
- Cálculo dinâmico de linhas por altura disponível (rows fixo em 4 por ora).
- Visão de chat/log em modo mobile (sem sidebar, sem chat-notifications — jogador não vê resultados de rolagem sem sair do modo).
- O painel de status ativo de terceiros (`visual-active-effects`) fica visualmente na mesma área dos orbes; o z-index:101 resolve o CONFLITO de sobreposição, mas não reorganiza os dois pra nunca se tocarem geometricamente — coexistem, não colidem mais.

**Verificado ao vivo, fim a fim (world arton, GM2/Al Simmons, resize real do browser pra 375×812):** troca de aba (Perícias↔Poderes) funciona e persiste seleção; grid de Perícias pagina 1/2 corretamente; clique numa perícia abre o diálogo nativo "Configuração de Uso de Perícia" TOTALMENTE visível e clicável, "Usar Habilidade" posta a rolagem no chat (confirmado via `game.messages`, timestamp batendo com o clique); clique no orbe de PV abre o prompt, `-3` aplica dano real (`actor.update` confirmado, valor mudou de 42→39→e depois voltou a mostrar corretamente sem ficar atrás do painel de terceiros); resize de volta pra desktop restaura o layout normal sem nenhuma regressão (perícias+painel direito lado a lado, orbes nas pontas, tudo igual à Fase 8 original do Footer HUD).

### Modo mobile da HUD — Fase 3: aba "Mapa" (canvas visível) (v1.107.0)

Pedido do usuário: melhorar a visualização do canvas (mapa/tokens) em modo mobile — até aqui (Fases 1-2) o `#board` ficava simplesmente escondido o tempo todo. `MobileTabKey` ganhou `"mapa"` como 6ª aba (`state.ts`), ao lado de Perícias/Inventário/Poderes/Magias/Macros — sem grid própria (`buildMobileContentHtml` retorna `""` pra essa aba), só revela o canvas por baixo. De propósito, SEM scene-controls (é visualização — pan/pinch-zoom/tap em token já funcionam via touch nativo do Foundry, sem precisar da toolbar).

**⚠️ Investigação longa, 3 hipóteses erradas descartadas na ordem, achado real no final** — vale registrar o método, não só o resultado, porque cada hipótese "fazia sentido" e falhou por um motivo específico só visível ao vivo:

1. **1ª hipótese (errada): `#board` não sobrevive a um ciclo esconder→reexibir via `display:none`.** Parecia óbvio — WebGL é notoriamente sensível a isso. Trocar pra `visibility:hidden` (elemento "vivo", sem motivo pra navegador descartar a layer) foi tentado a fundo, com `canvas.app.resize()`/`.render()` em loop via `requestAnimationFrame`, `canvas.perception.update()` e até `await canvas.draw()` (redesenho completo) — nada mudava a tela, sempre uma cor sólida.
2. **2ª hipótese (errada): fog-of-war/vision, já que `game.user.isGM` é `false` pro cliente de teste** (GM2 é role TRUSTED, não GAMEMASTER — achado à parte, relevante pra qualquer teste futuro nessa conta). Descartada rápido: `canvas.visibility.visible = false` (desliga a camada de névoa inteira) não mudou nada, e o desktop mostra o mapa perfeito pro MESMO usuário/cena.
3. **Achado decisivo — não é o `#board` que quebra, é o BUFFER que nunca é desenhado, E só quando `#hotbar` cobre a tela inteira.** O truque foi parar de confiar na tela (screenshot) e ler o BUFFER real do canvas: `board.toDataURL('image/png')` → desenhar num canvas 2D offscreen → `getImageData()` em pontos aleatórios. Resultado: RGB(0,0,0) em toda amostra — nem a cor "errada" que aparecia na tela, um preto puro, mostrando que a TELA e o BUFFER real eram coisas DIFERENTES (evidência de um problema de composição do navegador, não dos nossos dados). Bisecção sistemática (reativar cada regra nossa uma de cada vez via `element.style.setProperty(prop, val, "important")` ao vivo, sem rebuild) isolou a causa: com `#hotbar.t20-footer-hud { display:none }` via DevTools, o mapa aparecia PERFEITO na hora — mesmo sem tocar `#board` em nada. A causa real: um elemento `position:fixed; inset:0` cobrindo o viewport INTEIRO (mesmo com fundo transparente) faz o navegador tratar tudo por baixo como "ocluído" e suspender a composição daquela camada — não é um bug do Foundry nem dos nossos dados, é o compositor do navegador otimizando de forma agressiva demais pra esse padrão específico.
4. **Fix:** na aba "Mapa", `#hotbar` deixa de ser `inset:0;height:100dvh` e encolhe pra `inset:auto 0 0 0;height:auto` — só a faixa da barra de abas embaixo, sem cobrir o resto da tela. Cabeçalho/banner de turno/conteúdo (que já é vazio nessa aba) ficam `display:none`. Sem elemento nenhum cobrindo o viewport inteiro, o navegador para de ocluir o canvas e ele volta a desenhar normalmente — confirmado ao vivo com reload limpo (canvas NUNCA escondido/mostrado, sempre visível, só coberto pelo fundo opaco da HUD nas outras abas via `background:var(--t20-bg-deepest)` — decisão final: nunca mais tocar em display/visibility do `#board`, só cobrir visualmente).
- **Verificado ao vivo, reload limpo (world arton, GM2/Al Simmons, 375×812):** aba "Mapa" mostra o mapa colorido completo com tokens, `#board` nunca passou por display:none/visibility:hidden nesse fluxo; trocar de volta pra "Perícias" restaura a HUD cheia sem artefato; diálogo de perícia nativo continua abrindo por cima corretamente (sem regressão dos fixes de z-index da Fase 2); resize de volta pra desktop sem regressão.

### Modo mobile da HUD — Fase 4: barras laterais (scene-controls + sidebar) (v1.108.0)

Pedido do usuário: acesso às "duas barras laterais" (esquerda = `#scene-controls`, ferramentas; direita = `#sidebar`, chat/combate/atores/itens/diário/tabelas/decks/playlists/compêndios/config) em modo mobile, mantendo boa usabilidade.

- **Decisão de design — não são duas abas novas, é uma aba nova + um complemento:** `#scene-controls` não faz sentido sozinho (as ferramentas agem SOBRE o canvas — medir, desenhar, posicionar token), então passou a ser revelado JUNTO da aba "Mapa" já existente (Fase 3), não uma aba própria. `#sidebar` é auto-contido (chat, combate, etc. — nada a ver com canvas) e virou uma aba nova, **"Chat"** (6ª aba, `MobileTabKey` ganhou `"chat"`), ícone `fa-comments`. Resolve de quebra o gap já documentado da Fase 2 ("sem chat visível em modo mobile") — a aba Chat mostra a sidebar NATIVA completa, com as próprias abas internas (Combate, Atores, Itens, Diário, etc.) intactas — nada foi reconstruído, só revelado.
- **Generalização do mecanismo da Fase 3:** `applyMobileMapActiveClass` virou `applyMobileRevealClass(tab)` — mapeia `MobileTabKey → classe no <html>` (`mapa→t20-mobile-map-active`, `chat→t20-mobile-chat-active`) e desliga todas as outras ao trocar de aba (só uma ativa por vez). CSS combinado via `:is(.t20-mobile-map-active, .t20-mobile-chat-active)` pros efeitos comuns (esconder cabeçalho/conteúdo, encolher `#hotbar` pra só a tabbar).
- **`#scene-controls`/`#sidebar` são DOM comum, sem WebGL** — ao contrário do `#board`, `display:none` condicional simples é seguro pra eles (nenhum dos problemas de compositor que afetaram o canvas se aplica). Continuam escondidos por padrão e revelados só na aba certa.
- **⚠️ Dois bugs reais adicionais, mesma FAMÍLIA do bug da Fase 3 (`transform` nativo atrapalhando posicionamento), achados ao vivo em `#sidebar`:**
  1. **`position:fixed` no `#sidebar` não resolvia pro viewport real** — media `x:1050` numa tela de 375px. Causa: o ANCESTRAL `#ui-right` tem `transform:translateX(750px)` E o PRÓPRIO `#sidebar` tem `transform:translateX(300px)` (posicionamento nativo calculado pra layout desktop). Por spec CSS, um ancestral com `transform` vira o "containing block" de descendentes `position:fixed` — o fixed resolvia relativo a essa caixa deslocada, não ao viewport. Fix: `transform:none !important` nos DOIS (`#ui-right` e `#sidebar`).
  2. **`#sidebar` é 100% transparente por padrão** (`background-color: rgba(0,0,0,0)`) — só fica opaco onde o CONTEÚDO de cada aba interna pinta o próprio fundo. A aba Chat (cheia de bolhas de mensagem) mascarava isso bem, mas trocar pra Combate (conteúdo curto, "Sem Encontros") revelava o mapa por baixo nos espaços vazios. Fix: `background:var(--t20-bg-deepest) !important` no `#sidebar` revelado, cobrindo TODAS as abas internas por igual, não só as com conteúdo denso.
- **Altura reservada pra tabbar:** `#sidebar` revelado usa `bottom: var(--t20-hud-height)` (a mesma CSS var que a Fase 2 já calcula a cada render via `#updateHudHeightVar()` — na aba Chat a HUD já encolheu pra só a tabbar ANTES dessa medição rodar, então a var já reflete a altura certa sem cálculo extra).
- **Verificado ao vivo, reload limpo (world arton, GM2/Al Simmons, 375×812):** aba "Chat" mostra a sidebar nativa completa e opaca (chat log com mensagens reais — inclusive cards de distribuição de tesouro/entrega de item de sessões anteriores —, dice tray, campo de mensagem); trocar pra aba interna "Combate" (`button[data-tab="combat"]`) funciona e mostra "Sem Encontros" com fundo opaco correto (sem o mapa vazando); aba "Mapa" mostra `#scene-controls` (grade de ferramentas) sobreposto ao canvas, no canto esperado; voltar pra "Perícias" restaura a HUD cheia; resize de volta pra desktop (com o setting em "auto") confirma `#sidebar` volta ao native `position:static`/transparente/posição normal, sem nenhum resíduo dos overrides mobile.

### Barra de Buffs & Condições (v1.109.0)

`src/hud/buffs.ts` — chips de ícone dos Active Effects atualmente relevantes do ator, minimizável. Desktop: 2ª linha do `.t20-hud-panel`, logo ACIMA de tudo (retrato/perícias/painel direito) — o painel virou `flex-direction:column` (`.t20-hud-buffs-bar` + `.t20-hud-panel-row`, este último com o conteúdo antigo intacto). Mobile: linha própria (`.t20-hud-mobile-buffs-bar`) logo abaixo do cabeçalho.

- **⚠️ Achado ao vivo (motivo do filtro não ser trivial):** a 1ª versão filtrava só `disabled`/`isSuppressed`/`transfer:true`/`onuse` — e mostrava lixo real (Al Simmons, verificado): "Magia Instintiva", "Caminho do Arcanista: Feiticeiro", "Aumento de Atributo - Sabedoria", "Dom da Esperança", "Arcano de Batalha". Causa: o T20 copia pra `actor.effects` **praticamente todo poder/traço de classe** com `transfer:false` + sem `statuses` + sem duração — a MESMA forma de um buff temporário de verdade. Não existe campo nativo que distinga "traço permanente" de "buff temporário".
- **Filtro final (`isVisibleBuff`):** mostra só quando `effStatuses(e).length>0` (condição de status — sempre temporária por definição) **OU** `flags.<MODULE_ID>` tem QUALQUER chave (`hasOwnModuleFlag` — sinal de que um SUBSISTEMA NOSSO criou/tagueou o effect de propósito: `duration-manager` [DUR_FLAG], Consagrar/Aura Sagrada/Égide Sagrada, Inspiração, Orientação, Concentração de Combate, Aço-Rubi, etc.). Exclui por completo as cópias nativas passivas do T20 (nunca carregam flag nossa). Guards adicionais: `disabled`, `isSuppressed`, `transfer:true`, `onuse` (Efeito de Uso, só existe no momento da rolagem).
- **Tooltip com duração:** quando o effect tem o `dur` flag do `duration-manager` (`formatDurationLabel`), o tooltip ganha o sufixo (`"N rodada(s)"`/`"até o fim da cena"`/`"até passar 1 dia"`/`"sustentada"`). Buffs de área (Consagrar/Aura/Égide) não têm esse flag (o duration-manager deliberadamente não os gerencia — `hasAnyModuleFlag` os pula) e aparecem sem sufixo, só o nome.
- **Minimizável:** botão chevron+contador (`data-buffs-toggle`) — client setting `hud.buffsCollapsed` (compartilhado entre desktop/mobile, mesmo ator/jogador). Colapsado, some a lista de chips mas o toggle+contador continuam visíveis pra reabrir. Sem NENHUM buff visível, o builder retorna `""` — nem a borda/toggle aparecem (nada a minimizar).
...
- **Trigger:** `createChatMessage` com o MESMO idioma de detecção do `auto-damage` (mensagem cujos `rolls` têm `type:"attack"` E `type:"damage"` — identifica qualquer ataque com arma, nativo ou não) de um ator com o poder, cujo `data-item-id` NÃO resolve pra uma das próprias Patas (evita reagir ao ataque extra que ELE MESMO dispara — sem essa guarda, fora de combate, onde o gate de rodada não existe, entraria em loop), e que ainda não foi ofertado nesta rodada. Abre um `Dialog` simples sim/não ("Sim (2 PM)" / "Não atacar") — **custo FLAT, não por perna** (ver bug v1.113.3 abaixo).
...
- **Execução:** confirmado o prompt, `actor.spendMana(2)` **uma única vez** + `weapon.roll({})` nas DUAS Patas — como é uma arma NATIVA (rolls `type:"ataque"`/`type:"dano"` no formato padrão), o próprio `createChatMessage` do `auto-damage/index.ts` (já registrado, roda antes/depois independente da ordem) processa cada ataque exatamente como qualquer outra arma: RD, reações, aplicação de dano no(s) alvo(s) marcado(s) em `game.user.targets`, crítico x2 em 20 natural nativo. **Nenhum código de dano/RD/aplicação foi escrito neste módulo.**
...
**⚠️ BUG v1.109.0→v1.110.0 (real, resolvido) — dano/acerto da Pata vazava bônus de OUTRAS armas equipadas** (relatado com Aller: uma Manopla equipada com a melhoria "Precisa" aplicava o bônus no ataque da Pata Inseto — usuário citou de memória como "Certeira"; o item real na ficha é "Manopla Precisa"/melhoria `precise`, mesma classe de bug independente do nome exato). Achado lendo `tormenta20.mjs` diretamente (`AbilityUseDialog.create` E o branch `configureDialog:false` de `Item.prototype.roll`): os DOIS caminhos nativos de `roll()` juntam `item.actor.effects.filter(ae => ae.getFlag("tormenta20","onuse") && ae.flags.tormenta20[tipo])` **sem checar `ae.origin === item.uuid`** — ao contrário da Manopla (que filtra por `origin` manualmente, `t20-fixes/manopla-upgrades.ts`). Como aprimoramentos de arma (ex.: "Precisa") são copiados pro `actor.effects`/`item.effects` e ficam habilitados persistentemente (não são um checkbox por-rolagem), QUALQUER ataque do ator poderia pegá-los se o filtro de origem não existisse. Fix (`rollPataAttackClean`): não chama `weapon.roll()` de jeito nenhum (nem com dialog nem sem) — chama `clone.rollAttack({options:{}})` + `clone.rollDamage({options})` + `clone.displayCard({options,...})` DIRETO no clone da Pata (os mesmos métodos internos que `Item.roll()` usa por baixo). Nenhum dos três lê `actor.effects` — só `itemData.rolls` (a fórmula fixa da Pata) + o `options` vazio que passamos — e `displayCard` ainda posta o MESMO chat card nativo (`flags.tormenta20.itemData` etc.) que `auto-damage` já sabe processar, então nada mais no fluxo (RD/reações/aplicação de dano no alvo T) precisou mudar.
- **⚠️ BUG v1.110.0→v1.110.1 (real, resolvido, achado na verificação ao vivo) — `rollAttack` lançava `Cannot set properties of undefined (setting 'Ataque')`:** `Item.prototype.roll` nativo faz `item.system.rolled = {}` antes de chamar `rollAttack`/`rollDamage` (que fazem `itemData.rolled[r.name] = roll`) — como `rollPataAttackClean` pula `roll()` inteiro, esqueceu de inicializar `clone.system.rolled` também. Fix: uma linha (`clone.system["rolled"] = {}`) antes das duas chamadas. **Verificado ao vivo fim-a-fim (Aller Brushfighter, world arton, GM2):** atacar com "Ataque desarmado" (Manopla Precisa equipada, bônus "Precisa" +5/+0 aplicado corretamente nesse ataque) → prompt "Membros Extras" → "2 ataques extras" → as DUAS Patas rolam `1d20+5+0+0` / `1d4[corte]` cada, **sem `onUseEffects`/`effects` nenhum no card** (zero vazamento, confirmado comparando os 3 cards lado a lado) — PM debitado corretamente (2×2=4).
- **Ícone bundled + migração (v1.111.0):** ícone genérico do Foundry (`icons/skills/melee/strike-hand-fist-yellow-brown.webp`) trocado pelo artwork dedicado do usuário — `public/assets/Items/pata-inseto.png` (fonte original 2048×2048 em `Data/assets/Items/Pata Inseto - Membros Extras.png`, redimensionada pra 512×512 antes de empacotar — ícone de item não precisa de resolução de retrato). `PATA_IMG` agora aponta pro caminho module-relative (`modules/t20-theme-overhaul/assets/Items/pata-inseto.png`). **Migração** (`migratePataIcons`, `Hooks.once("ready")`, só nos atores que o cliente possui — mesmo padrão do `manopla-upgrades.ts`): atualiza o `img` de Patas já existentes na ficha (criadas antes do ícone bundled) via `updateEmbeddedDocuments`, idempotente. Criação/remoção "as duas patas juntas" já era o comportamento existente desde v1.103.2 (`grantPatas`/`removePatas` sempre operam em UM `createEmbeddedDocuments`/`deleteEmbeddedDocuments` com as 2 patas de uma vez, nunca uma de cada vez) — confirmado nesta verificação, não precisou de mudança. **Verificado ao vivo (Aller Brushfighter):** migração corrigiu o ícone das 2 patas já existentes ao carregar o mundo; remover o poder "Membros Extras" apagou as 2 patas juntas; re-adicionar (item real do compêndio `tormenta20.poderes`) recriou as 2 patas juntas, já com o ícone novo.
- **⚠️ BUG v1.111.0→v1.113.3 (real, resolvido) — custo tratado como por-perna, deveria ser FLAT:** a leitura original do texto do poder ("gasta 2 PM para fazer um ataque extra com cada uma") virou um prompt 0/1/2 pernas a 2/4 PM — o usuário corrigiu: os 2 PM destravam o ataque extra com AS DUAS patas de uma vez, nunca uma perna avulsa nem 4 PM. Fix: `computeMaxLegsByPm` (0-2 pernas pagáveis) virou `canAffordBothAttacks` (bool, só checa se há PM pro custo flat); `promptLegs` (picker 0/1/2) virou `promptBothAttacks` (dialog sim/não); `offerExtraAttacks` debita `spendMana(2)` **uma vez só**, fora do loop, e sempre rola as DUAS Patas quando confirmado.
...
- **`scripts/gen-pocoes-pergaminhos.mjs`** (committed, reexecutável): lê o harvest → `packs-src/pocoes-pergaminhos/*.json`.
  ...
  - **Poção**: só magias elegíveis por alvo (`isEligibleForPotion`: exclui `alcance==="spec"`; exclui `alcance==="none"` SEM `area`; **`alcance==="self"` sempre elegível, com ou sem área** — corrigido em v1.110.0, ver abaixo). **1 variante base + 1 por CADA aprimoramento individual** (decisão do usuário: sem combinar 2+, evita explosão combinatória — power-set completo chegaria a milhares). Resultado real (pós-fix v1.110.0): 259 poções base + 726 variantes de aprimoramento = 985 poções, distribuição de 0 a 16 aprimoramentos por magia (outliers: Resistência à Energia 16, Conjurar Monstro 12 — contagem alta mas genuína, cada "escolha de elemento/tamanho" é uma AE onuse própria no compêndio nativo).
...
**Elegibilidade de Poção + botão "Criar Pergaminho" na conjuração (v1.110.0):**
- **Fix de regra (pedido do usuário):** `isEligibleForPotion` excluía `alcance:"self"` sem área — mas magia de alvo pessoal TAMBÉM "tem como alvo uma criatura" (o próprio conjurador), então deveria ser elegível SEMPRE, não só quando tem área. Corrigido (`scripts/gen-pocoes-pergaminhos.mjs`): `self` agora sempre elegível; só `none` continua exigindo área (magia verdadeiramente sem alvo nenhum). +59 magias pessoais viraram elegíveis (Invisibilidade, Voo, Metamorfose, Campo de Força, Visão Mística, etc.) — pack recompilado: 259 poções base (era 200) + 726 variantes de aprimoramento (era 569) = 985 poções (era 769). Pergaminhos inalterados (261, sem restrição de alvo).
- **Botão "Criar Pergaminho" na janela de lançar magia** (`src/pocoes-pergaminhos/craft.ts`): o T20 nativo já injeta, NESSE MESMO dialog (`AbilityUseDialog.create`), um botão "Preparar Poção" quando `actor.getFlag("tormenta20","createPotion")` (ou GM) — ver `T20.BrewPotion` no `tormenta20.mjs`. O equivalente pra pergaminho (`createScroll`, MESMA convenção nativa — checkbox própria em "Configurações do Personagem", `T20.ShowWriteScroll`) só existia nativamente como botão na FICHA da magia (`_createScroll`/`_getHeaderButtons`), fora do fluxo de conjuração. Fix: hook `renderAbilityUseDialog` injeta um `<button>` extra no `.dialog-buttons` do MESMO dialog quando `item.type==="magia"` e `(actor.getFlag("tormenta20","createScroll") || game.user.isGM)` — herda o styling cinematográfico automaticamente (`t20-dialog.css` já estiliza genericamente QUALQUER `<button>` dentro de `.dialog-buttons`/`.ability-use-form`, sem precisar de CSS novo).
  - **Poção (nativa, inalterada):** continua embutindo os aprimoramentos MARCADOS no formulário no momento da fabricação (`options.effects` do `applyOnUseEffects` nativo) — já era o comportamento correto, não mexemos.
  - **Pergaminho (novo):** clique próprio (não passa pelo `resolve()`/`submit()` nativo — chama `app.close()` no final, resolvendo a Promise nativa como `null`, igual outras features do módulo que patcheiam este dialog) → debita PM (`Math.max(custo,1)`, mesma fórmula nativa da poção) → cria um item com a MESMA forma dos itens do compêndio (`flags.pocaoPergaminho`, `kind:"pergaminho"`, **`identificado:true`** — o conjurador acabou de fabricar, não é loot, sem mascarar) → posta card. Nenhum aprimoramento é fixado (`aprimoramentoName:null`) — o patch existente de `Item.prototype.roll` re-busca a magia REAL via `spellUuid` a cada uso, com o picker completo disponível pra quem LER o pergaminho depois escolher e pagar (regra pedida pelo usuário: "pergaminhos o jogador precisa usar os aprimoramentos ele mesmo").
- **⚠️ BUG v1.110.0→v1.110.1 (real, resolvido) — `app.item` no `renderAbilityUseDialog` é um CLONE com `.id`/`.uuid` `null`:** achado ao vivo (Al Simmons, Adaga Mental) — o `spellUuid` gravado no pergaminho fabricado vinha **string vazia**, porque `Item.prototype.roll` nativo passa `this.clone({keepId:true})` pro `AbilityUseDialog.create` e, nesta versão do Foundry/T20, o clone tem `.id`/`._id`/`.uuid` `null` (MESMO achado já documentado no bug v1.104.0→v1.104.1 do patch de identificação — reincide aqui porque `craft.ts` lê `app.item` de um hook DIFERENTE, `renderAbilityUseDialog`, que também recebe o clone). Fix: `resolveRealSpell(actor, clone)` busca o item REAL em `actor.items` por nome (único campo que sobrevive íntegro no clone, junto com `actor`/`type`) antes de montar o flag do pergaminho.
- **Membros Extras** (fix de dano/acerto vazando bônus de outra arma) — ver seção própria acima.
- **Verificado ao vivo fim-a-fim (Al Simmons, world arton, GM2), pós-fix v1.110.1:** com `flags.tormenta20.createScroll` setada no ator, o dialog de "Adaga Mental" mostra `["Lançar Magia","Criar Pergaminho"]` (sem "Preparar Poção" — actor sem `createPotion`, confirma que as duas gates são independentes); clicar "Criar Pergaminho" cria "Pergaminho de Adaga Mental" com `spellUuid` correto (`Actor.<id>.Item.<id>` real) e `identificado:true`; usar o pergaminho reabre o dialog nativo "Configuração de uso de magia" com TODOS os aprimoramentos disponíveis (da magia + pessoais do ator — Magia Instintiva/Tomo do Rancor/Energético), confirmando reaproveitamento real da automação existente. ⚠️ **Gotcha de teste, não é bug:** cancelar o dialog nesse ponto AINDA consome o pergaminho — `handleCast` chama `consumeOne` incondicionalmente após `spell.roll()` resolver (mesmo cancelada), comportamento PRÉ-EXISTENTE de todos os itens `pocoes-pergaminhos` (não é novo nesta sessão).
