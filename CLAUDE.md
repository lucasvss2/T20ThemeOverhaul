...
- **vitest exclui `.claude/**`** — worktrees antigos do Claude carregavam cópias velhas dos testes contra o src ATUAL (via alias `@`), inflando/quebrando a suíte. Contagem real: 702 testes / 43 arquivos.
...
- **Aplicar:** reduz o **custo EFETIVO** do alvo em 1 — `system.ativacao.custo` **E** o custo dos "Efeitos de Uso" `flags.tormenta20.custo` (v1.75.2). ⚠️ Poderes de perícia/ataque (ex.: **Audácia**) cobram o PM pelo EFEITO onuse, não pelo `ativacao.custo` — os modais de teste leem o efeito; reduzir só `ativacao.custo` mostrava 1 na lista mas cobrava 2 no uso. `powerEffectiveCusto = max(ativacao, custos dos efeitos)`; elegibilidade usa o efetivo. **Sem AE** (não mexe em passivos/atributos). Flag `economiaHabilidade = {linkedItemId, originalCusto, reducedCusto, effectCustos:[{effectId,original,reduced}]}` + **renomeia SÓ esta instância** para `Economia de Habilidade (nome do alvo)` (`economiaDisplayName`, v1.75.1). Migração `reconcileActor` (ready, owner) conserta vínculos antigos que só reduziram `ativacao.custo`.
