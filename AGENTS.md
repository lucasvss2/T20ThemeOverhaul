...
npm test            # vitest run — 656 tests must pass (vitest exclui .Codex/** — worktrees antigos poluem a suíte)
...
src/
  ...
  hud/skill-icons.ts           — 29 ícones SVG de perícia portados do Codex Design (por label PT-BR); ícone "Ofício" = cinzel/martelo (v1.97.0)
...
- **vitest exclui `.Codex/**`** — worktrees antigos do Codex carregavam cópias velhas dos testes contra o src ATUAL (via alias `@`), inflando/quebrando a suíte. Contagem real: 714 testes / 44 arquivos.
...
- **Animação — 1x por seta:** `fireSetaAnimations` chama `AutomatedAnimations.playAnimation(sourceToken, item, {targets:[alvoDaSeta]})` uma vez por seta (staggered ~180ms, mesma API que `baforada/index.ts` usa), cada uma mirando o alvo REAL que a seta recebeu — inclusive no caso de 1 alvo só (N vezes pro mesmo alvo). ⚠️ **Ao contrário da Baforada, o cast nativo NÃO é cancelado** (o picker de aprimoramentos do T20 continua intacto — reimplementar essa UI não valeria o custo só pra controlar a animação) — isso significa que o Automated Animations PODE disparar automaticamente por conta própria durante o cast nativo, ANTES do nosso disparo manual; não há como suprimir esse disparo nativo sem cancelar o cast inteiro (mutar `flags.autoanimations` do clone antes do dialog nativo não helps — o débito/roll/card/trigger do A-A acontecem DEPOIS que nosso wrapper de `AbilityUseDialog.create` já devolveu o controle, fora do nosso alcance). Overshoot de 1 disparo extra é o risco conhecido, aceito por ora — precisa confirmação ao vivo.
...
- **`scripts/gen-pocoes-pergaminhos.mjs`** (committed, reexecutável): lê o harvest → `packs-src/pocoes-pergaminhos/*.json`.
  ...
  - **Regenerar**: colher o harvest ao vivo de novo (script no console, GM) → salvar em `C:\Users\lucas\.Codex\plans\t20-harvest-magias.json` → `node scripts/gen-pocoes-pergaminhos.mjs` → `npm run build:packs`.
